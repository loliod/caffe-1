...
class SMDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        self.sbdd_dir = params['dir']
        self.split = params['split']
        self.random = params.get('randomize', True)
        self.seed = params.get('seed', None)
	self.scale= params.get('scale',None)
    ...
    def reshape(self, bottom, top):
        ...
        top[0].reshape(1,1,1,len(self.data))
        top[1].reshape(1)
    ...
    def load_data(self, idx):
        ...
        """
        data = np.load('{}'.format(idx))
	return  data[:-1],int(data[-1])


class GPDataLayer(caffe.Layer):
    """
    Load (input image, label image) pairs from the SBDD extended labeling
    of PASCAL VOC for semantic segmentation
    one-at-a-time while reshaping the net to preserve dimensions.
    ...
    def setup(self, bottom, top):
        ...
        # config
        params = eval(self.param_str)
        self.sbdd_dir = params['voc_dir']
        self.split = params['split']
        self.mean = np.array(params['mean'])
        self.random = params.get('randomize', True)
        self.seed = params.get('seed', None)
        self.scale= params.get('scale',None)
	self.iscolor=params.get('iscolor',True)
    ...
    def load_data(self, idx):
        ...
        """
        import scipy.io
        mat = scipy.io.loadmat('{}'.format(idx))
	...
	if not self.iscolor:
        ...
        data -= self.mean
	if self.scale:
		data *=float(self.scale)
        data = data.transpose((2,0,1))
        label = mat['label'].astype(np.uint8)
        label = label[np.newaxis, ...]
        return data,label

class SantiagoDataLayer(caffe.Layer):
    """
    ...
    def reshape(self, bottom, top):
        # load image + label image pair
        filename,label=self.indices[self.idx].split(' ')
        self.data = self.load_image(filename)
        self.label = int(label)
        # reshape tops to fit (leading 1 is for batch dimension)
        #rtop[0].reshape(1, *self.data.shape)
